import {NgModule} from '@angular/core';
import {CommonModule} from '@angular/common';

import {LoadingModule} from "ngx-loading";
import {NgxDatatableModule} from "@swimlane/ngx-datatable";
import {DateTimePickerModule} from "ng-pick-datetime";
import {MyDatePickerModule} from 'mydatepicker';
import {FormsModule} from "@angular/forms";
import {ChartsModule} from "ng2-charts";
import {NotificationsService, SimpleNotificationsModule} from "angular2-notifications/dist";
import {FacturasProveedorRoutingModule} from "./facturas-proveedor-routing.module";
import {FacturasGastosComponent} from "./facturas-gastos/facturas-gastos.component";
import {FacturasGastosPagarComponent} from "./facturas-gastos-pagar/facturas-gastos-pagar.component";
import {FacturasGastosTransitoPagoComponent} from "./facturas-gastos-transito-pago/facturas-gastos-transito-pago.component";
import {FacturasGastosPagadasComponent} from "./facturas-gastos-pagadas/facturas-gastos-pagadas.component";
import {FacturasProveedorComponent} from "./facturas-proveedor.component";


@NgModule({
  imports: [
    CommonModule,
    FacturasProveedorRoutingModule,
    LoadingModule,
    NgxDatatableModule,
    MyDatePickerModule,
    DateTimePickerModule,
    SimpleNotificationsModule.forRoot(),
    FormsModule,
    ChartsModule,
  ],
  declarations: [
    FacturasProveedorComponent,
    FacturasGastosComponent,
    FacturasGastosPagarComponent,
    FacturasGastosTransitoPagoComponent,
    FacturasGastosPagadasComponent,

  ],
  entryComponents: [

  ],
  providers: [
    NotificationsService,
  ]
})
export class FacturasProveedorModule { }
