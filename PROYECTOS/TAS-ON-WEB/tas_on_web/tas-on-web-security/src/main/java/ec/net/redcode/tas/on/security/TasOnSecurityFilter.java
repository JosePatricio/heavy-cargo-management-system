...
public class TasOnSecurityFilter extends AbstractAccessTokenValidator implements ContainerRequestFilter{
    ...
    public void filter(ContainerRequestContext containerRequestContext) throws IOException {
        supportedSchemes.add(OAuthConstants.BEARER_AUTHORIZATION_SCHEME);
    ...
    protected void validateRequest(Message m) {
        try {
            ...
            TasOnErrorMessage message = new TasOnErrorMessage(Response.Status.FORBIDDEN.getStatusCode(), "Error validating token", e.getLocalizedMessage());
    ...
    protected String getTokenFromFormData(Message message) {
        ...
        if (type != null && MediaType.APPLICATION_JSON.startsWith(type)
    ...
    protected String validateAudiences(List<String> audiences) {
        if (StringUtils.isEmpty(audiences) && getAudience() == null) {
            return null;
        }
        if (getAudience() != null) {
        ...
        if (!isAudienceIsEndpointAddress()) {
            return null;
        }
        String requestPath = (String) PhaseInterceptorChain.getCurrentMessage().get(Message.REQUEST_URL);
        for (String s : audiences) {
            boolean matched = isCompleteAudienceMatch() ? requestPath.equals(s) : requestPath.startsWith(s);
            if (matched) {
                return s;
            }
        }
    ...
    protected SecurityContext createSecurityContext(HttpServletRequest request,
        ...
        final UserSubject theSubject =
                TasOnSecurityFilter.this.useUserSubject ? resourceOwnerSubject : clientSubject;
    ...
    public void setAm(AuthenticationMethod am) {
        this.am = am;
    }
